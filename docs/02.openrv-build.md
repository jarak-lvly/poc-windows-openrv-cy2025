...
## Verify installation
After the container is up and running, you can remote in using RDP and review the logs to see if there were any errors with package installation:  `C:\OEM\openrv-cy2025-install.log`.  If you don’t, then pay attention to `rvcfg` / `rvbootstrap` where it will mostly likely complain about missing paths / packages.
...
## Example Output 
Example console output (truncated) from a successful build is included in: docs/openrv-build-example-output.txt.
